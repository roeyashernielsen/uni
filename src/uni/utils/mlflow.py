...
from src.uni.io import PathType
from src.uni.io.reader import load
...
def log_artifact(artifact_md: ArtifactMD):
    ...
    if artifact_md.path_type == PathType.Directory:
        ...
        mlflow.log_param(f"{artifact_md.name}_path",
        ...
        mlflow.log_param(f"{artifact_md.name}_path",
                         os.path.join(mlflow.get_artifact_uri(artifact_md.name),
                                      artifact_md.name + artifact_md.file_format))
    else:
        raise ValueError(f"path_type must be a PathType but got {type(artifact_md.path_type)}")
